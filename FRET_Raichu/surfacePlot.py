...
def surfacePlot(Hmap, nrows, ncols, xyspacing, zscale, name, hRange, file_path, lutfromfile, lut, lut_file_path, colorbar_on, save, show):
    ...
    #Create a new figure
    mlab.figure(size=(512, 512))
    mlab.figure(size=(520, 696)) #torsten's other camera
    #mlab.figure(size=(494, 494))    
    ...
    if save:
        mlab.savefig(file_path, size=(512, 512))
        if show == False:
            mlab.close()
